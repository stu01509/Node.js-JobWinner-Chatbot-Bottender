...
const yoctolCase = require('./func/case');
const welcome = require('./func/welcome');

const search = require('./func/search.js');

const job = require('./func/job');


const select = require('./func/select');
...
bot.onEvent(
  ...
  btdc.condition([
    ...
    // 搜尋 Wanted
    [btdc.isPayloadMatch('searchWanted'), btdc.sendText(
      context =>{
        if(context.event.text == 'Wanted'){
          btdc.sendText('選擇 Wanted')
          return `你選擇, ${context.event.text}`;
        }else if(context.event.text == 'Dcard'){
          btdc.sendText('選擇 Dcard')          
          return  `你選擇, ${context.event.text} 2.`;
        }
      }      
    )],

    // 搜尋 Dcard
    [btdc.isPayloadMatch('searchDcard'), btdc.sendText('選擇 Dcard')],

 


    [btdc.isTextMatch(/\/*工作/), btdc.sendText(
      context =>
      `${context.session.user.first_name} ${
          context.session.user.last_name
        }, QQQQ.${context.event.text}`
    )],
    ...
    [btdc.isTextMatch(/射手/), btdc.sendText('看來粗心大意的射手座，內心其實是理想主義、十分高遠。射手座適合做基金會、慈善事業、社會運動。他們興趣很廣，由於工作運勢不錯，各種行業的潛力新星，都可看到射手座的身影。他們也適合高等教育、宗教、跨國企業經營、海外旅遊、媒體、投資、清潔能源相關的事業。要小心得意忘形。 ')],
    [btdc.isTextMatch(/魔羯/), btdc.sendText('很多政治鬥爭下的存活者。他們夠低調、超級務實、會用不停做事來掩飾自己的權力欲望。魔羯座很容易得到大人物的信任，適合長期規劃的建設案、各種私募基金、風險資金、私人銀行工作、家族信託基金。另外，魔羯座作為法官、員警、律師、社會工作人員、公務員、教授的也很多，循規蹈矩的努力執行，讓他們有機會爬到高位。要避免利慾薰心。')],
    ...
    [btdc.isTextMatch(/誰是世界上最(美麗|漂亮|迷人|正)的女[人孩生]/),
      btdc.sendText('K K')
    ],
    ...
    [btdc.isTextMatch(/有錢/),
      btdc.series([
        btdc.sendText('學生 還是社會人士?'),
        btdc.sendImage('https://i.imgur.com/39P4Wl9.jpg'),
        btdc.sendButtonTemplate('按鈕Demo', [{
            type: 'postback',
            title: 'More about Dino',
            payload: 'More_Dino'
          },
          {
            type: 'postback',
    ...
    [
      btdc.isPayloadMatch('誰最有錢'),
      btdc.random([btdc.sendText('Dino'), btdc.sendText('周良賢'), btdc.sendText('Richno')])
    ],

    [
      btdc.isTextMatch('誰最窮'),
      btdc.sendText(
        'Pick a color:', {
          quick_replies: [{
            content_type: 'text',
    ...
    [
      btdc.isPayloadMatch('__BOTTENDER__'),
      btdc.sendButtonTemplate('以下是 Bottender 的相關資源', [{
          type: 'web_url',
          title: 'Bottender 官網',
    ...
    [
      btdc.isPayloadMatch('__JOIN_YOCTOL__'),
    ...
    [btdc.isTextMatch(/優拓.*案例/), yoctolCase],
    // [
    //   context => true,
    //   btdc.random([btdc.sendText('我聽不懂這句'), btdc.sendText('我不知道該怎麼回')]),
    // ],
    // [
    //   context => true,
    //   btdc.random([btdc.sendText('早安'), btdc.sendText('安安')]),
    // ]
  ]),
  btdc.condition([
    [btdc.isTextMatch('誰最有窮'), btdc.sendText('Cliff')],
...
//   console.log('User Send' + context.event.text);
 
//   if(context.event.text == 'Dcard') {
//     job = 'Dcard';
//     await context.sendText('你選擇 Dcard');
//   }else if(context.event.text == 'Wanted') {
//     job = 'Wanted';
//     await context.sendText('你選擇 Wanted');
//   }

//   if(context.event.text === '抓'){
//     await context.sendText(job);
//   }
// });
